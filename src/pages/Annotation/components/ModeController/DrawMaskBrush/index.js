...
const DrawMaskBrush = () => {
  ...
  const getCurrentAnnotationImageId = useDatasetStore(
    (state) => state.getCurrentAnnotationImageId
  );

  const getRenderingSize = useGeneralStore((state) => state.getRenderingSize);
  const updateCurrentMousePosition = useGeneralStore(
    (state) => state.updateCurrentMousePosition
  );
  const getCurrentMousePosition = useGeneralStore(
    (state) => state.getCurrentMousePosition
  );

  const getSelectedObjectId = useAnnotationStore(
    (state) => state.getSelectedObjectId
  );
  const getAnnotationByAnnotationObjectId = useAnnotationStore(
    (state) => state.getAnnotationByAnnotationObjectId
  );
  const appendAnnotation = useAnnotationStore(
    (state) => state.appendAnnotation
  );
  const setAnnotationWithImageId = useAnnotationStore(
    (state) => state.setAnnotationWithImageId
  );
  const deleteAnnotation = useAnnotationStore(
    (state) => state.deleteAnnotation
  );
  const setSelectedObjectId = useAnnotationStore(
    (state) => state.setSelectedObjectId
  );
  const getOrCreateSelectedObjectId = useAnnotationStore(
    (state) => state.getOrCreateSelectedObjectId
  );
  ...
  const getIsDrawingScribble = useMaskBrushStore(
    (state) => state.getIsDrawingScribble
  );
  const setIsDrawingScribble = useMaskBrushStore(
    (state) => state.setIsDrawingScribble
  );

  const getCurrentAnnotation = async (createIfNotExist = true) => {
    if (getisLoading("create_new_annotation"))
      return null
    const objectId = await getOrCreateSelectedObjectId(
      instanceId,
      ENUM_ANNOTATION_TYPE.MASK,
      {
    ...
    const drawingAnnotation = getAnnotationByAnnotationObjectId(
      objectId,
      annotationImageId
    );
    if (drawingAnnotation) {
      ...
      console.log("create_new_annotation");
      const toolConfig = getToolConfig();
      console.log(toolConfig.threshold);
      const newAnnotation = new MaskAnnotationClass(
        "",
        objectId,
        annotationImageId,
        {
  ...
  const handleStartDrawByBrush = async () => {
    ...
    maskBrushes.push({
      points: [
        [
          currentMousePosition.x / imageWidth,
          currentMousePosition.y / imageHeight,
        ],
      ],
    ...
    setAnnotationWithImageId(
      drawingAnnotation.id,
      drawingAnnotation.annotationImageId,
      { maskBrushes },
  ...
  const handleFinishDrawByBrush = async () => {
    ...
    const newMaskBlob = await drawBrushToMask(maskBitmap, maskBrushes, {
    ...
    setAnnotationWithImageId(
      drawingAnnotation.id,
      drawingAnnotation.annotationImageId,
      cloneDeep(drawingAnnotation.maskData),
  ...
  const handleDeleteAnnotation = () => {
    ...
    const annotationImageId = getCurrentAnnotationImageId();
    const currentAnnotation = getAnnotationByAnnotationObjectId(
      currentObjectId,
      annotationImageId
    );
  ...
  useEffect(() => {
    ...
    let subscriptions = {
      [EVENT_TYPES.STAGE_DRAG_START]: getSubject(
        EVENT_TYPES.STAGE_DRAG_START
      ).subscribe({ next: (e) => handleDragStart(e) }),
      [EVENT_TYPES.STAGE_MOUSE_DOWN]: getSubject(
        EVENT_TYPES.STAGE_MOUSE_DOWN
      ).subscribe({ next: (e) => handleMouseDown(e) }),
      [EVENT_TYPES.STAGE_TOUCH_START]: getSubject(
        EVENT_TYPES.STAGE_TOUCH_START
      ).subscribe({ next: (e) => handleMouseDown(e) }),
      [EVENT_TYPES.STAGE_MOUSE_MOVE]: getSubject(
        EVENT_TYPES.STAGE_MOUSE_MOVE
      ).subscribe({ next: (e) => handleMouseMove(e) }),
      [EVENT_TYPES.STAGE_TOUCH_MOVE]: getSubject(
        EVENT_TYPES.STAGE_TOUCH_MOVE
      ).subscribe({ next: (e) => handleMouseMove(e) }),
      [EVENT_TYPES.STAGE_MOUSE_UP]: getSubject(
        EVENT_TYPES.STAGE_MOUSE_UP
      ).subscribe({ next: (e) => handleMouseUp(e) }),
      [EVENT_TYPES.STAGE_TOUCH_END]: getSubject(
        EVENT_TYPES.STAGE_TOUCH_END
      ).subscribe({ next: (e) => handleMouseUp(e) }),
      [EVENT_TYPES.STAGE_MOUSE_ENTER]: getSubject(
        EVENT_TYPES.STAGE_MOUSE_ENTER
      ).subscribe({ next: (e) => handleMouseEnter(e) }),
      [EVENT_TYPES.UNSELECT_CURRENT_ANNOTATION_OBJECT]: getSubject(
        EVENT_TYPES.UNSELECT_CURRENT_ANNOTATION_OBJECT
      ).subscribe({ next: (e) => handleUnselectCurrentAnnotationObject(e) }),
      [EVENT_TYPES.EDIT.DELETE_ANNOTATION]: getSubject(
        EVENT_TYPES.EDIT.DELETE_ANNOTATION
