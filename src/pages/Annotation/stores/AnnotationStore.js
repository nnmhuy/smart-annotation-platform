...
const useAnnotationStore = create((set, get) => ({
  ...
  getOrCreateSelectedObjectId: async (
    dataInstanceId,
    annotationType,
    properties = {},
    attributes = {}
    ...
    const setIsLoading = get().setIsLoading;
    setIsLoading("loading_object_info", true);
    const selectedObjectId = get().selectedObjectId;
    const annotationObjects = get().annotationObjects;
    let res;
    if (selectedObjectId) {
      res = selectedObjectId;
    } else {
      setIsLoading("create_new_annotation", true);
      console.log("Create new object");
      const newAnnotationObject = new AnnotationObjectClass(
        "",
        dataInstanceId,
        "",
        annotationType,
      ...
      res = newAnnotationObject.id;
    }
    setIsLoading("loading_object_info", false);
    return res;
  },
  ...
  deleteAnnotationObject: (objectId) => {
    ...
    Object.keys(newAnnotations).forEach((annotationImageId) => {
      newAnnotations[annotationImageId] = filter(
        newAnnotations[annotationImageId],
        (ann) => ann.annotationObjectId !== objectId
      );
    });
    newAnnotationObjects = filter(
  ...
  loadAnnotations: async (instanceId) => {
    ...
    let annotationsObj = await AnnotationService.getAnnotationsByDataInstance(
      instanceId
    );
    let annotations = {};
  ...
  setAnnotation: async (
    ...
    Object.keys(annotations).forEach((annotationImageId) => {
      annotations[annotationImageId] = annotations[annotationImageId].map(
        (annotation) => {
  ...
  setAnnotationWithImageId: async (
    annotationId,
    annotationImageId,
    newEditingAnnotationData,
    options = {}
  ...
  updateAnnotations: async (newAnnotationsDict, options = {}) => {
    ...
    try {
      if (commitAnnotation) {
        await Promise.all(
          Object.keys(newAnnotationsDict).map((id) =>
            newAnnotationsDict[id].applyUpdate()
          )
        );
      }
    } catch (error) {
  ...
  getAnnotationByAnnotationObjectId: (
    annotationObjectId,
    annotationImageId
  ) => {
    const setIsLoading = get().setIsLoading;
    let annotations = get().annotations;
    let res = find(annotations[annotationImageId], { annotationObjectId });
    return res;
  },
  deleteAnnotation: (deleteAnnotationId, options = {}) => {
    ...
    Object.keys(annotations).forEach((annotationImageId) => {
      annotations[annotationImageId] = filter(
        annotations[annotationImageId],
  ...
  setDrawingAnnotation: (newDrawingAnnotation) =>
    set({ drawingAnnotation: newDrawingAnnotation }),
  ...
  updatePropagatedAnnotations: async (newAnnotations, options = {}) => {
    ...
    newAnnotations.every((newAnnotation, index) => {
      const pos = findIndex(annotations[newAnnotation.annotationImageId], {
        id: newAnnotation.id,
      });
    ...
    if (commitAnnotation) {
      await Promise.all(
        newAnnotations.slice(0, breakFrameIndex).map((ann) => ann.applyUpdate())
      );
    }
